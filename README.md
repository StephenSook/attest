...
1. A run is created with the directory record it verifies; the run id doubles as the CALL-E `Idempotency-Key`, so an identical retry returns the original call instead of dialing again. Before the external request, Attest stores only a keyed HMAC-SHA256 request digest, an attempt count, a ten-minute cutoff, and an owner-token lease. It never persists the raw destination for recovery. If a response is lost, the same browser request can safely retry after the lease ends. Attest refuses changed credentials or provider endpoints, concurrent attempts, and late retries.
