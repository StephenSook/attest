...
- Before CALL-E receives a request, Attest persists the directory organization and claims, provider and sandbox metadata, the endpoint and credential fingerprint, hashes binding the client capability, complete run request, destination, and provider dispatch, plus the attempt count, cutoff, and lease. It does not persist the raw phone number or full raw dispatch request. If a response is lost, an identical client request may reuse the original idempotency key after the lease ends. A changed request, credential rotation, provider switch, concurrent attempt, or late retry fails closed. After any ambiguous first attempt, a later 4xx cannot release the sandbox reservation because it does not prove the original call was rejected.
